...
const statusEntries: Array<[FilterStatus, string]> = [['pending', '\u5f85\u5ba1\u6838'], ['approved', '\u5f85\u53d1\u653e'], ['completed', '\u5df2\u5b8c\u6210'], ['rejected', '\u5df2\u9a73\u56de'], ['cancelled', '\u5df2\u53d6\u6d88']];
...
export function ManagerView({ options, token, role = 'manager', panel = 'queue', actorId, onRequireRelogin }: { options: AppOptions; token?: string; role?: Role; panel?: Panel; actorId?: string; onRequireRelogin?: () => void }) {
  ...
  return <section className="workspace manager-workspace"><div className="page-heading manager-heading"><div><p className="eyebrow">{role === 'super_admin' ? '超级管理' : '管理工作台'}</p><h1>{panel === 'users' ? '账号管理' : panel === 'archive' ? '常规记录' : panel === 'reissue' ? '物资发放记录' : panel === 'ready' ? '待完成' : '待审核'}</h1></div>{panel !== 'users' && <div className="manager-metrics"><div><span>待审核</span><strong>{pendingCount}</strong></div><div><span>待完成</span><strong>{approvedCount}</strong></div><div><span>已完成</span><strong>{issuedCount}</strong></div></div>}</div>
    ...
    {panel === 'users' ? <UserAdminView token={token} actorRole={role} actorId={actorId} onRequireRelogin={onRequireRelogin} /> : loading ? <div className="empty-state"><LoaderCircle className="spin" size={24} /></div> : <><RequestList panel={panel} groups={panel === 'queue' ? visibleQueue : panel === 'ready' ? visibleReady : panel === 'reissue' ? visibleReissue : visibleArchive} options={options} role={role} onAction={mutate} />{error && <FloatingNotice kind="error" text={error} onDismiss={() => setError('')} actionLabel="重试" onAction={() => void load()} />}</>}
...
function StatusFilters({ value, onChange }: { value: FilterStatus[]; onChange: (value: FilterStatus[]) => void }) {
  ...
  return <div className="filter-choice-group status-filter-group" role="group" aria-label="筛选状态"><span className="filter-choice-label"><Layers3 size={14} />状态</span><button type="button" className={value.length === statusEntries.length ? 'filter-choice selected' : 'filter-choice'} onClick={() => onChange(statusEntries.map(([status]) => status))}>全部</button>{statusEntries.map(([status, label]) => <button type="button" className={value.includes(status) ? 'filter-choice selected' : 'filter-choice'} key={status} onClick={() => toggle(status)}>{label}</button>)}</div>;
}

function RequestList({ panel, groups, options, role, onAction }: { panel: 'queue' | 'ready' | 'archive' | 'reissue'; groups: ManagerGroup[]; options: AppOptions; role: Role; onAction: (action: 'approve' | 'reject' | 'issue' | 'complete', group: ManagerGroup) => void }) {
  const [expandedId, setExpandedId] = useState<string | null>(null);
  useEffect(() => setExpandedId(null), [panel]);
  if (!groups.length) return <div className="empty-state"><div className="empty-icon"><Check size={22} /></div><h3>{panel === 'queue' ? '审核队列已清空' : panel === 'ready' ? '暂无待完成申请' : panel === 'reissue' ? '暂无物资发放记录' : '暂无常规记录'}</h3><p>{panel === 'queue' ? '新的客服申请会出现在这里' : panel === 'ready' ? '管理通过后，申请会出现在这里' : '可调整筛选条件查看历史'}</p></div>;
  ...
  return <div className="manager-groups">{[...map.entries()].map(([key, items]) => { const start = sequence; sequence += items.length; return <div className="server-section" key={key}><div className="record-table-shell manager-record-table" role="table" aria-label={panel === 'ready' ? '待完成申请列表' : panel === 'reissue' || panel === 'queue' ? '物资申请列表' : '申请列表'}><RecordTableHeader commandMode={commandMode} actor={actor} isReissue={isReissueTable} /><div className="record-table-body" role="rowgroup">{items.map((group, index) => <RequestCard key={group.id} index={start + index + 1} panel={commandMode ? 'ready' : panel} group={group} options={options} role={role} expanded={expandedId === group.id} onToggle={() => setExpandedId((current) => current === group.id ? null : group.id)} onAction={onAction} />)}</div></div></div>; })}</div>;
}
function RequestCard({ index, panel, group, options, role, expanded, onToggle, onAction }: { index: number; panel: 'queue' | 'ready' | 'archive' | 'reissue'; group: ManagerGroup; options: AppOptions; role: Role; expanded: boolean; onToggle: () => void; onAction: (action: 'approve' | 'reject' | 'issue' | 'complete', group: ManagerGroup) => void }) {
  ...
  const firstCommand = displayCommands[0]?.text;
  ...
  return <article className={`manager-card record-card manager-record status-card-${group.status} ${expanded ? 'is-expanded' : ''}`}>
      ...
      <div className={`record-cell record-reason ${commandMode ? 'record-command-preview' : ''}`} data-label={commandMode ? '指令' : '理由'} role="cell" title={commandMode ? firstCommand : reason}>
        {commandMode ? firstCommand ? <><code>{firstCommand}</code><span onClick={(event) => event.stopPropagation()} onKeyDown={(event) => event.stopPropagation()}><CopyButton text={firstCommand} label="复制指令" /></span>{group.commands.length > 1 && <em className="record-plus">+{group.commands.length - 1}</em>}</> : '--' : <span>{reason}{group.rejectionReason ? ` · ${group.rejectionReason}` : ''}</span>}
      </div>
    ...
    {expanded && <div className="record-card-content">
      {panel === 'archive' ? <WorkflowTimeline group={group} /> : panel === 'reissue' ? <><WorkflowTimeline group={group} /><IssuanceItemsDisplay group={group} /></> : panel === 'queue' ? <IssuanceItemsDisplay group={group} /> : commandMode ? <CommandDetails commands={displayCommands} /> : <>
...
function CommandDetails({ commands }: { commands: GeneratedCommand[] }) {
  return <div className="command-list">{commands.map((command) => <CommandRow command={command} key={`${command.operationIndex}-${command.sequence}`} />)}</div>;
}

function CommandRow({ command }: { command: GeneratedCommand }) {
