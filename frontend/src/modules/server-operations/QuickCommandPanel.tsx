...
import type { ConfiguredServer } from './types';
...
export function QuickCommandPanel({ server, sending, disabled, onSend }: Props) {
  return <section className="server-quick-command-panel" aria-label="快捷指令参数">
    <div className="server-quick-command-heading">
      <strong>当前服务器配置</strong>
      <span>修改下方参数后逐项发送；自动刷新不会覆盖输入。</span>
    </div>
    <div className="server-quick-command-grid">
...
function QuickCommandRow({ command, server, sending, disabled, onSend }: Props & { command: Command }) {
  ...
  return <form className="server-quick-command-row" aria-label={`${command.label}快捷指令`} onSubmit={submit} noValidate>
    <div className="server-quick-command-summary">
      <strong>{command.label}</strong>
      <span className="server-quick-command-reference">{command.fields.map(([key]) => `${referenceInput(server[key]) || '未配置'} ${fieldUnit(key)}`).join(' · ')}</span>
    </div>
    <div className="server-quick-command-controls">
      <div className="server-quick-command-inputs">
      ...
      <button type="button" className="secondary-button server-quick-command-reset" aria-label="恢复当前配置" title="恢复当前配置" disabled={sending} onClick={reset}><RotateCcw size={14} aria-hidden="true" /></button>
    ...
    {error && <p id={`${id}-error`} className="setup-error" role="alert">{error}</p>}
  </form>;
}
