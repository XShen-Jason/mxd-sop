...
import { accountCharacterLabel, accountIsActive, accountStatusLabel, type ConfiguredServer, type ServerAccount, type ServerMessageType } from './types';
...
interface ServerDetailsPanelProps {
  ...
  onAddAccount: () => void;
  busy: boolean;
  onToggleAutomation: (account: ServerAccount) => void;
...
export function ServerDetailsPanel({ server, busy, onAddAccount, onToggleAccount, onToggleAutomation, onEditAccount, onDeleteAccount, onReconnectAccount, onSendMessage, statusFresh }: ServerDetailsPanelProps) {
  const status = serverLiveStatus(server, statusFresh);
  return <section className="server-detail-panel panel-surface" aria-labelledby="selected-server-title">
    <header className="server-detail-header">
      ...
      <div className="server-detail-actions"><span className="server-detail-status server-live-summary" data-status-tone={status.tone}><span className="server-live-dot" data-status-tone={status.tone} />{status.label}</span></div>
    </header>
    <section className="server-accounts-section" aria-labelledby="server-accounts-title">
      ...
      <div className="server-account-list" role="list">{server.accounts.length ? server.accounts.map((account) => <AccountRow key={account.id} account={account} serverId={server.id} live={statusFresh} busy={busy} onAutomation={onToggleAutomation} onToggle={onToggleAccount} onEdit={onEditAccount} onDelete={onDeleteAccount} onReconnect={onReconnectAccount} />) : <div className="server-account-empty server-detail-account-empty"><UserRound size={19} /><div><strong>暂未配置账号</strong><span>使用上方“添加账号”开始登录并选择角色。</span></div></div>}</div>
    </section>
    <ServerMessagePanel key={server.id} server={server} statusFresh={statusFresh} onSend={onSendMessage} />
  </section>;
}

function AccountRow({ account, serverId, live, busy, onAutomation, onToggle, onEdit, onDelete, onReconnect }: { account: ServerAccount; serverId: string; live: boolean; busy: boolean; onAutomation: (account: ServerAccount) => void; onToggle: ServerDetailsPanelProps['onToggleAccount']; onEdit: (account: ServerAccount) => void; onDelete: (account: ServerAccount) => void; onReconnect: (account: ServerAccount) => void }) {
  ...
  return <div className="server-account-row" role="listitem">
    ...
    <div className="server-account-row-actions"><button type="button" className="icon-button" title="编辑账号" aria-label={`编辑 ${account.username}`} onClick={() => onEdit(account)}><Pencil size={15} /></button><button type="button" className="icon-button danger-button" title="删除账号" aria-label={`删除 ${account.username}`} onClick={() => onDelete(account)}><Trash2 size={15} /></button>{reconnectable && <button type="button" className="icon-button" title="手动重连" aria-label={`重连 ${account.username}`} onClick={() => onReconnect(account)}><RefreshCw size={15} /></button>}<label className="account-toggle automation-toggle" title="仅启用的在线账号参与自动化；关闭不会退出登录"><input type="checkbox" role="switch" aria-label={`自动化 ${account.username}`} checked={account.automation_enabled === true} disabled={!live || busy} onChange={() => onAutomation(account)} /><span className="account-toggle-track"><i aria-hidden="true" /><b>自动化</b></span></label><button type="button" className="secondary-button account-login" disabled={!live || busy} aria-label={`${active ? '退出' : '登录'} ${account.username}`} onClick={() => onToggle(serverId, account.id, !active)}><Power size={13} />{active ? '退出' : '登录'}</button></div>
  </div>;
}
