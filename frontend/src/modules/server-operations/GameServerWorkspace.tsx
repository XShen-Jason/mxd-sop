...
import { serverHealth, type AccountSetupValue, type ConfiguredServer, type ServerAccount, type ServerFormValue, type ServerMessageType } from './types';
...
export function GameServerWorkspace({ options, userId, token, connectionEnabled, connectionRevision = 0 }: { options: AppOptions; userId?: string; token?: string; connectionEnabled: boolean; connectionRevision?: number }) {
  ...
  const saveAccount = async (value: AccountSetupValue) => {
    ...
    const input = { username: value.username, ...(value.password ? { password: value.password } : {}), character_id: value.characterId, character_name: value.characterName, enabled: value.enabled };
    const action = account ? () => client.updateAutoAccount(server.id, account.id, input) : () => client.createAutoAccount(server.id, { ...input, password: value.password, ...(value.sessionId ? { session_id: value.sessionId } : {}) });
  ...
  const toggleAccount = (serverId: string, accountId: string, enabled: boolean) => {
    ...
    void run(`account-${accountId}`, () => client.startAutoAccount(serverId, accountId), '账号启动请求已发送');
  };
  const confirmAction = () => {
    ...
    if (action.kind === 'account-stop') void run(`account-${action.account.id}`, () => client.stopAutoAccount(action.server.id, action.account.id), '账号已关闭');
  };
  ...
  return <section className="game-server-workspace" aria-labelledby="game-server-page-title">{settings}{error && <FloatingNotice kind="error" text={error} onDismiss={() => setError('')} actionLabel="重试" onAction={() => { void loadStatus(true); void loadOverview(); }} />}{notice && <FloatingNotice kind="success" text={notice} onDismiss={() => setNotice('')} />}<section className="server-management-layout" aria-label="游戏服务器管理"><section className="server-list-panel panel-surface" aria-labelledby="game-server-page-title"><div className="server-list-heading"><div><h2 id="game-server-page-title">服务器列表</h2><span>状态来自 mxd-auto-process，每 2 秒同步一次。</span></div><button type="button" className="primary-button server-configure-button" disabled={!canAddServer || loading || Boolean(busy)} onClick={() => { setEditingServer(null); setServerDialogOpen(true); }}><ServerCog size={18} />配置服务器</button></div>{loading ? <div className="server-empty"><LoaderCircle className="spin" size={24} /><span>正在读取 auto 数据…</span></div> : <div className="server-list">{servers.length ? servers.map((server) => <ServerListItem key={server.id} server={server} live={statusFresh} selected={selected?.id === server.id} busy={Boolean(busy)} onSelect={() => setSelectedId(server.id)} onEdit={() => { setEditingServer(server); setServerDialogOpen(true); }} onDelete={() => setPendingAction({ kind: 'server', server })} />) : <div className="server-empty"><ServerCog size={24} /><strong>还没有游戏服务器</strong><span>点击配置服务器并选择一个服务器标识。</span></div>}</div>}</section>{selected ? <ServerDetailsPanel server={selected} statusFresh={statusFresh} onAddAccount={() => setAccountTarget({ server: selected })} onToggleAccount={toggleAccount} onEditAccount={(account) => setAccountTarget({ server: selected, account })} onDeleteAccount={(account) => setPendingAction({ kind: 'account-delete', server: selected, account })} onReconnectAccount={reconnect} onSendMessage={sendMessage} /> : <div className="server-detail-empty panel-surface"><ServerCog size={25} /><strong>选择或配置一台服务器</strong><span>服务器配置完成后，可在这里管理账号并发送消息。</span></div>}</section>{serverDialogOpen && <ServerConfigDialog options={options} server={editingServer} configuredIds={servers.map((server) => server.id)} onCancel={() => { setEditingServer(null); setServerDialogOpen(false); }} onSave={(value) => void saveServer(value)} />}{accountTarget && <AccountSetupDialog server={accountTarget.server} account={accountTarget.account} onCancel={cancelAccountSetup} onResetLogin={resetAccountSetup} onLogin={(username, password) => client.startAutoSession(accountTarget.server.id, { account: username, password })} onEnterGame={(sessionId, characterId) => client.selectAndEnterAutoSession(sessionId, characterId)} onComplete={(value) => saveAccount(value)} />}{pendingAction && <ConfirmDialog danger busy={Boolean(busy)} title={pendingTitle(pendingAction)} description={pendingDescription(pendingAction)} confirmLabel={pendingAction.kind === 'account-stop' ? '确认关闭' : '确认删除'} onCancel={() => setPendingAction(null)} onConfirm={confirmAction} />}</section>;
}
...
function ServerListItem({ server, live, selected, busy, onSelect, onEdit, onDelete }: { server: ConfiguredServer; live: boolean; selected: boolean; busy: boolean; onSelect: () => void; onEdit: () => void; onDelete: () => void }) {
  const health = live ? serverHealth(server) : 'pending';
  return <article className={`server-list-item ${selected ? 'selected' : ''}`}><button type="button" className="server-list-select" onClick={onSelect} aria-pressed={selected}><span className={`server-status-dot health-${health}`} aria-hidden="true" /><span className="server-list-copy"><strong>{server.name}</strong><span>{server.address}</span></span>{selected && <Check className="server-list-check" size={17} aria-label="当前选择" />}</button><div className="server-list-item-actions"><button type="button" className="icon-button" title="修改服务器" aria-label={`修改 ${server.name}`} onClick={onEdit} disabled={busy}><Pencil size={16} /></button><button type="button" className="icon-button danger-button" title="删除服务器" aria-label={`删除 ${server.name}`} onClick={onDelete} disabled={busy}><Trash2 size={16} /></button></div></article>;
}

function formatAddress(host: string, port: number) { return host.includes(':') ? `[${host}]:${port}` : `${host}:${port}`; }
function pendingTitle(action: PendingAction) { return action.kind === 'server' ? `删除 ${action.server.name}？` : action.kind === 'account-delete' ? `删除账号 ${action.account.username}？` : `关闭账号 ${action.account.username}？`; }
function pendingDescription(action: PendingAction) { return action.kind === 'server' ? '删除后 auto 中的服务器配置和其账号都会被移除。' : action.kind === 'account-delete' ? '删除后 auto 中保存的加密凭据和运行会话都会被移除。' : '关闭会退出该账号并断开与游戏服务器的连接；之后不会自动重新登录。'; }
