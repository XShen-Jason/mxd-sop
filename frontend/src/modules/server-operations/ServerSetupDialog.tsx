...
import { normalizeGameHost, normalizeGamePort, type AccountSetupValue, type ConfiguredServer, type ServerAccount, type ServerFormValue } from './types';
...
export function ServerConfigDialog({ options, server, configuredIds, onCancel, onSave }: { options: AppOptions; server: ConfiguredServer | null; configuredIds: string[]; onCancel: () => void; onSave: (value: ServerFormValue) => void }) {
  ...
  const [form, setForm] = useState<ServerFormValue>({ catalogId: server?.id ?? availableServers[0]?.id ?? '', host: initial.host, port: initial.port });
  ...
  const submit = (event: FormEvent) => {
    ...
    setFormError('');
    onSave({ catalogId: form.catalogId, host, port: String(port) });
  };

  return <div className="server-dialog-backdrop" role="presentation"><div className="server-dialog" role="dialog" aria-modal="true" aria-labelledby="server-dialog-title"><div className="server-dialog-header"><div><h2 id="server-dialog-title">{server ? '修改游戏服务器' : '添加游戏服务器'}</h2></div><button type="button" className="icon-button" aria-label="关闭" onClick={onCancel}><X size={18} /></button></div><form className="server-dialog-form" onSubmit={submit}><label><span>服务器标识</span><select required value={form.catalogId} onChange={(event) => setForm({ ...form, catalogId: event.target.value })} disabled={Boolean(server)}><option value="" disabled>选择服务器</option>{availableServers.map((item) => <option value={item.id} key={item.id}>{item.displayName}</option>)}{server && !availableServers.some((item) => item.id === server.id) && <option value={server.id}>{server.name}</option>}</select></label><div className="server-form-grid tcp-address-grid"><label className="server-endpoint-field"><span>游戏服务器 IP</span><input required type="text" inputMode="url" value={form.host} placeholder="127.0.0.1" onChange={(event) => { setForm({ ...form, host: event.target.value }); setFormError(''); }} /><small>auto 将通过 TCP 连接此地址</small></label><label className="server-endpoint-field"><span>端口</span><input required type="number" inputMode="numeric" min="1" max="65535" value={form.port} placeholder="12660" onChange={(event) => { setForm({ ...form, port: event.target.value }); setFormError(''); }} /><small>游戏服务端口</small></label></div>{formError && <p className="setup-error" role="alert">{formError}</p>}<div className="server-dialog-actions"><button type="button" className="secondary-button" onClick={onCancel}>取消</button><button type="submit" className="primary-button"><Check size={16} />保存配置</button></div></form></div></div>;
}
