...
export function AccountSetupDialog({ server, account, onCancel, onResetLogin, onLogin, onEnterGame, onComplete }: { server: ConfiguredServer; account?: ServerAccount | null; onCancel: (sessionId?: string) => void; onResetLogin: (sessionId: string) => Promise<void>; onLogin?: (username: string, password: string, credentialType: AutoCredentialType) => Promise<AutoSession>; onEnterGame?: (sessionId: string, characterId: string) => Promise<AutoSession>; onComplete: (value: AccountSetupValue) => Promise<boolean> | boolean }) {
  ...
  const initialCredentialType: AutoCredentialType = account?.credential_type ?? 'password';
  const [step, setStep] = useState<1 | 2 | 3>(1);
  const [username, setUsername] = useState(account?.username ?? '');
  const [password, setPassword] = useState('');
  const [credentialType, setCredentialType] = useState<AutoCredentialType>(initialCredentialType);
  ...
  const login = async (event: FormEvent) => {
    ...
    if (credentialType === 'md5' && password && !/^[0-9a-fA-F]{16}$/u.test(password)) return setError('请输入 16 位十六进制 MD5 登录值');
    if (editing && credentialType !== initialCredentialType && !password) return setError('切换密码类型时请输入新的密码值');
    setError('');
    if (editing) return setStep(2);
    if (!onLogin) return setError('登录服务未配置');
    setActionLoading(true);
    try {
      const result = await onLogin(username.trim(), password, credentialType);
  ...
  const complete = async () => {
    ...
    setError('');
    try {
      const saved = await onComplete({ username: username.trim(), password, credentialType, characterId: characterId.trim(), characterName: characterName.trim(), enabled: account?.enabled ?? true, ...(sessionId ? { sessionId } : {}) });
