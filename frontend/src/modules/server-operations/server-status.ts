import type { ConfiguredServer, ServerAccount } from './types';

export type StatusTone = 'ready' | 'waiting' | 'error' | 'inactive';
export interface LiveStatus { tone: StatusTone; label: string }

export function accountLiveStatus(account: ServerAccount, serverEnabled: boolean, fresh: boolean): LiveStatus {
...
export function serverLiveStatus(server: ConfiguredServer, fresh: boolean): LiveStatus {
  if (!fresh) return { tone: 'waiting', label: '状态待确认' };
