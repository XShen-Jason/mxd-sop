...
export function ActivityView({ role = 'manager', token }: { role?: Role; token?: string }) {
  ...
  const save = () => {
    ...
    const itemCodes = rewards.filter((reward) => reward.kind === 'item').map((reward) => reward.itemCode);
    if (new Set(itemCodes).size !== itemCodes.length) { setNotice({ kind: 'error', text: '同一物品不能重复添加' }); return; }
    const next = { ...draft, id: editing?.id ?? (globalThis.crypto?.randomUUID?.() ?? `${Date.now()}-${Math.random()}`), name: cleanName, description: draft.description.trim(), rewards, updatedAt: new Date().toISOString() };
  ...
  const chooseItem = (item: CatalogItem) => {
    ...
    if (draft.rewards.some((reward) => reward.kind === 'item' && reward.itemCode === item.code)) { setNotice({ kind: 'error', text: '同一物品不能重复添加' }); return; }
  ...
  const selectedItemCodes = new Set(draft.rewards.filter((reward) => reward.kind === 'item' && reward.itemCode).map((reward) => reward.itemCode));
...
function ActivityEditor({ draft, setDraft, setReward, onSave, onCancel }: { draft: Activity; setDraft: (value: Activity | ((current: Activity) => Activity)) => void; setReward: (index: number, next: Partial<ActivityReward>) => void; onSave: () => void; onCancel: () => void }) {
  ...
  return <section className="panel-surface form-panel activity-editor">
    ...
    {itemRewards.map(({ reward, index }) => <div className="activity-fixed-reward item-reward-row" key={index}><span className="activity-item-name">{reward.itemName || '请从右侧选择道具'}</span><input inputMode="numeric" aria-label="道具数量" value={reward.quantity || ''} onChange={(event) => setReward(index, { quantity: quantity(event) })} /><button type="button" className="icon-button danger-button" aria-label="删除道具奖励" onClick={() => setDraft((current) => ({ ...current, rewards: current.rewards.filter((_, rewardIndex) => rewardIndex !== index) }))}><X size={15} /></button></div>)}
