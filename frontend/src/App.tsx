...
export default function App() {
  ...
  useEffect(() => { if (!session || session.user.role === 'customer') { setWorkspaceCounts({}); return; } const loadCounts = async () => { try { const queue = await client.queue(); const next: Partial<Record<Workspace, number>> = { queue: queue.groups.filter((group) => group.status === 'pending').length }; if (session.user.role === 'super_admin') { const approved = await client.archive('approved'); next.ready = approved.groups.filter((group) => group.status === 'approved').length; } setWorkspaceCounts(next); } catch { /* panels expose errors */ } }; const refresh = () => { void loadCounts(); }; void loadCounts(); window.addEventListener('workspace-counts-refresh', refresh); return () => window.removeEventListener('workspace-counts-refresh', refresh); }, [session?.user.id, session?.user.role, client]);
