...
import { Archive, Bell, CalendarDays, ClipboardList, ContactRound, Headphones, KeyRound, ListTree, LoaderCircle, LogIn, LogOut, Menu, PackageCheck, ServerCog, ShieldCheck, SlidersHorizontal, UserRound, Wrench, X } from 'lucide-react';
...
const playerNavItems: NavItem[] = [{ id: 'player-directory', label: '玩家列表', icon: ContactRound }, { id: 'team-view', label: '所有队伍', icon: ListTree }];
const workspacePaths: Record<Workspace, string> = { request: '/request', records: '/records', 'player-directory': '/player-directory', 'team-view': '/team-view', 'player-integration': '/player-integration', reminders: '/reminders', activities: '/activities', queue: '/queue', ready: '/ready', archive: '/archive', reissue: '/reissue', accounts: '/accounts' };
...
function workspaceFromPath(role: Session['user']['role']): Workspace { const path = window.location.pathname.replace(/\/$/, ''); const found = (Object.entries(workspacePaths) as Array<[Workspace, string]>).find(([, value]) => value === path)?.[0]; if (!found) return defaultWorkspace(role); if (found === 'reminders' || found === 'player-directory' || found === 'team-view') return found; if (found === 'player-integration') return role === 'super_admin' ? found : defaultWorkspace(role); if (role === 'customer') return found === 'request' || found === 'records' ? found : 'request'; return found; }
function workspaceItems(role: Session['user']['role']): NavItem[] {
  ...
  if (role === 'customer') return items.concat(playerNavItems);
  items.push({ id: 'queue', label: '待审核', icon: Headphones });
  if (role === 'super_admin') items.push({ id: 'ready', label: '待完成', icon: PackageCheck });
  items.push({ id: 'reissue', label: '物资发放记录', icon: Archive }, { id: 'archive', label: '常规操作记录', icon: Archive }, { id: 'activities', label: '活动与道具', icon: CalendarDays }, ...playerNavItems, { id: 'accounts', label: '账号管理', icon: ShieldCheck });
  if (role === 'super_admin') items.push({ id: 'player-integration', label: '玩家服务', icon: ServerCog });
  return items;
}
