...
export class ApiClient {
  ...
  mine(cursor?: string) { const params = new URLSearchParams({ limit: '50' }); if (cursor) params.set('cursor', cursor); return this.request<Page<Group>>(`/api/v1/operation-groups/mine?${params}`); }
  cancel(id: string) { return this.request<Group>(`/api/v1/operation-groups/${encodeURIComponent(id)}/cancel`, { method: 'POST' }); }
  queue(serverId?: string, cursor?: string) { const params = new URLSearchParams({ limit: '100' }); if (serverId) params.set('serverId', serverId); if (cursor) params.set('cursor', cursor); return this.request<Page<ManagerGroup>>(`/api/v1/manager/operation-groups/queue?${params}`); }
  archive(status?: string, serverId?: string, cursor?: string) { const params = new URLSearchParams({ limit: '100' }); if (status) params.set('status', status); if (serverId) params.set('serverId', serverId); if (cursor) params.set('cursor', cursor); return this.request<Page<ManagerGroup>>(`/api/v1/manager/operation-groups/archive?${params}`); }
