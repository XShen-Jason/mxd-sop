...
首屏为登录页，不提供注册。普通客服只看到申请操作和我的申请工作区；管理和超级管理进入待审核、待完成、物资发放记录、常规操作记录及账号管理工作区，操作按钮按会话角色显示。
...
2. 客服我的申请：只能看到当前账号提交的记录，按提交时间倒序；筛选栏中的发物资/常规操作互斥，默认显示发物资；物资记录在 pending 审核前、常规操作记录在 approved 待完成前可修改或经二次确认后取消，不显示任何指令；默认状态展示待审核、待完成、已完成和已驳回，取消默认不选，刷新恢复默认筛选。
