...
- manager/super_admin 才能读取 queue/archive/overview、看到 commands 和审批；issue 仅 super_admin。活动配置替换仅 manager/super_admin，读取对所有已认证角色开放。账号删除通过 auth.delete-user 执行，并受当前账号与最后超管保护。
