...
`ActivitiesService`/`activities.list`/`activities.replace`（共享数据与权限）以及前端 `ActivityView` 和 `activities/store`（Activity、ActivityReward 类型及缓存读写函数）。

## Dependencies

activities.list、activities.replace、item-catalog.search、item-catalog.by-class、现有 CustomerView 申请表单和浏览器 localStorage 缓存。

## Data, configuration, and assets

后端 SQLite/JSON activity repository；前端存储键 `game-support-activities` 仅作缓存，复用 `game-support-recent-items` 最近道具记录。
