...
export function SendConfirmationDialog({ serverName, serverAddress, username, character, channel, message, busy, onCancel, onConfirm }: Props) {
  ...
  return <dialog ref={dialog} className="management-dialog management-send-confirmation" aria-labelledby="send-confirmation-title" aria-describedby="send-confirmation-description"
    onCancel={(event) => { event.preventDefault(); if (!busy) onCancel(); }}>
    <header className="management-dialog-heading"><h2 id="send-confirmation-title">确认发送？</h2>
    ...
    <strong className="management-send-content-label">发送内容</strong>
