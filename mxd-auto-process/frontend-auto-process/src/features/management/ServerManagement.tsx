...
export function ServerManagement({ onBack }: { onBack: () => void }) {
  ...
  const load = useCallback(async (showLoading = false) => {
    ...
    try {
      const value = await api.overview(controller.signal);
      ...
      if (!controller.signal.aborted) { setStatusFresh(false); setError(errorMessage(reason)); }
    } finally {
  ...
  const toggleAccount = (server: ServerRecord, account: AccountSnapshot) => { if (!account.enabled || window.confirm(`退出账号“${account.username}”？退出后停止重连，自动化设置保留。`)) void run(`account-${account.id}`, () => account.enabled ? api.stopAccount(server.id, account.id) : api.startAccount(server.id, account.id), account.enabled ? '账号已退出' : '账号登录请求已发送'); };
  const toggleAutomation = (server: ServerRecord, account: AccountSnapshot) => void run(`account-${account.id}`, () => api.updateAccount(server.id, account.id, { automation_enabled: !account.automation_enabled }), account.automation_enabled ? '自动化已关闭，登录状态不变' : '自动化已启用，在线时参与执行');
  ...
  return <section className="management-page" aria-labelledby="management-title">
    ...
    <div className="management-layout"><aside className="management-server-list panel-surface"><div className="management-list-heading"><div><strong>服务器与账号</strong><span>{servers.length} 台 · auto 独立数据</span></div><button type="button" className="icon-button" onClick={() => void load(true)} aria-label="刷新服务器列表" disabled={loading}><RefreshCw size={16} className={loading ? 'spin' : ''} /></button></div>{loading ? <div className="management-empty"><LoaderCircle size={24} className="spin" />正在读取 auto 配置…</div> : servers.length ? servers.map((server) => <ManagementServerRow key={server.id} server={server} live={statusFresh} selected={selected?.id === server.id} onSelect={() => setSelectedId(server.id)} onEdit={() => setDialog({ kind: 'server', server })} onDelete={() => void removeServer(server)} />) : <div className="management-empty"><ServerCog size={25} /><strong>尚未配置服务器</strong><span>点击右上角添加一台 auto 专属服务器。</span></div>}</aside>{selected ? <ManagementDetails server={selected} busy={busy} onAddAccount={() => setDialog({ kind: 'account', server: selected })} onEditAccount={(account) => setDialog({ kind: 'account', server: selected, account })} onDeleteAccount={(account) => void removeAccount(selected, account)} onToggleAccount={(account) => toggleAccount(selected, account)} onToggleAutomation={(account) => toggleAutomation(selected, account)} onReconnect={(account) => reconnect(selected, account)} onSendMessage={sendMessage} /> : <div className="management-detail-empty panel-surface"><ServerCog size={30} /><strong>选择或添加服务器</strong><span>服务器和账号数据完全保存在 auto 项目中。</span></div>}</div>
...
function ManagementServerRow({ server, live, selected, onSelect, onEdit, onDelete }: { server: ServerRecord; live: boolean; selected: boolean; onSelect: () => void; onEdit: () => void; onDelete: () => void }) {
  const status = serverLiveStatus(server, live);
  return <article className={`management-server-row${selected ? ' selected' : ''}`}><button type="button" className="management-server-select" onClick={onSelect} aria-pressed={selected}><span className="server-live-dot" data-server-status data-status-tone={status.tone} aria-label={status.label} /><span><strong>{server.name}</strong><code>{server.address}</code></span></button><div className="management-row-actions"><button type="button" className="icon-button" aria-label={`编辑 ${server.name}`} onClick={onEdit}><Pencil size={15} /></button><button type="button" className="icon-button danger-button" aria-label={`删除 ${server.name}`} onClick={onDelete}><Trash2 size={15} /></button></div></article>;
}

function ManagementDetails({ server, busy, onAddAccount, onEditAccount, onDeleteAccount, onToggleAccount, onToggleAutomation, onReconnect, onSendMessage }: { server: ServerRecord; busy: string; onAddAccount: () => void; onEditAccount: (account: AccountSnapshot) => void; onDeleteAccount: (account: AccountSnapshot) => void; onToggleAccount: (account: AccountSnapshot) => void; onToggleAutomation: (account: AccountSnapshot) => void; onReconnect: (account: AccountSnapshot) => void; onSendMessage: (server: ServerRecord, account: AccountSnapshot, mode: string, message: string) => Promise<AutoMessageResult | undefined> }) {
  return <section className="management-details panel-surface"><header className="management-details-heading"><div><p className="eyebrow">SERVER CONFIGURATION</p><h2>{server.name}</h2><code>{server.address} · 协议 {server.version} · 地图 {server.map_id}</code></div><span className={`management-enabled ${server.enabled ? 'on' : 'off'}`}>{server.enabled ? '已启用' : '已停用'}</span></header><section className="management-accounts"><div className="management-section-heading"><div><h3>已配置账号</h3><span>账号凭据只保存于 auto 本地数据库。</span></div><button type="button" className="primary-button" onClick={onAddAccount}><UserPlus size={16} />添加账号</button></div>{server.accounts.length ? <div className="management-account-list">{server.accounts.map((account) => <ManagementAccountRow key={account.id} account={account} busy={Boolean(busy)} onEdit={() => onEditAccount(account)} onDelete={() => onDeleteAccount(account)} onToggle={() => onToggleAccount(account)} onAutomation={() => onToggleAutomation(account)} onReconnect={() => onReconnect(account)} />)}</div> : <div className="management-account-empty"><UserRound size={21} /><span>尚未配置账号</span></div>}</section><MessagePanel key={server.id} server={server} busy={busy} onSend={onSendMessage} /></section>;
}

function ManagementAccountRow({ account, busy, onEdit, onDelete, onToggle, onAutomation, onReconnect }: { account: AccountSnapshot; busy: boolean; onEdit: () => void; onDelete: () => void; onToggle: () => void; onAutomation: () => void; onReconnect: () => void }) {
  const reconnectable = account.enabled && ['offline', 'failed'].includes(account.status);
  return <div className="management-account-row"><span className="management-account-icon"><UserRound size={16} /></span><div className="management-account-copy"><strong>{account.username}</strong><span>{account.character_name || '未命名角色'} · {account.character_id}</span></div><span className={`management-account-status status-${account.status}`}><i />{statusLabel(account.status)}</span><div className="management-account-actions"><button type="button" className="icon-button" aria-label={`编辑 ${account.username}`} onClick={onEdit} disabled={busy}><Pencil size={15} /></button><button type="button" className="icon-button danger-button" aria-label={`删除 ${account.username}`} onClick={onDelete} disabled={busy}><Trash2 size={15} /></button>{reconnectable && <button type="button" className="icon-button" aria-label={`重连 ${account.username}`} onClick={onReconnect} disabled={busy}><RefreshCw size={15} /></button>}<label className="management-automation-toggle" title="仅启用的在线账号参与自动化；关闭不会退出登录"><input type="checkbox" role="switch" aria-label={`自动化 ${account.username}`} checked={account.automation_enabled === true} disabled={busy} onChange={onAutomation} /><span><i aria-hidden="true" />自动化</span></label><button type="button" className={`account-power ${account.enabled ? 'enabled' : ''}`} onClick={onToggle} disabled={busy} aria-label={account.enabled ? `退出 ${account.username}` : `登录 ${account.username}`}>{busy ? <LoaderCircle size={15} className="spin" /> : <Power size={15} />}{account.enabled ? '退出' : '登录'}</button></div></div>;
}

function statusLabel(status: AccountSnapshot['status']) { return { online: '在线', connecting: '连接中', reconnecting: '重连中', offline: '离线', failed: '失败', disabled: '未登录' }[status]; }
