...
SELECT account_id,boss_type,character_id,day_key,joined_at FROM player_team_members WHERE team_id=? AND day_key=?`, sourceID, day); err != nil {
	...
	if _, err = tx.ExecContext(r.Context(), `UPDATE player_team_applications SET status='rejected',decided_at=? WHERE team_id=? AND status='pending'`, time.Now().UTC().Format(time.RFC3339), sourceID); err != nil {
		writeErr(w, errors.New("internal-error"))
		return
	}
	...
	result, err := a.loadTeam(r.Context(), targetID, ac.ID)
	if err != nil {
		writeErr(w, errors.New("internal-error"))
		return
	}
