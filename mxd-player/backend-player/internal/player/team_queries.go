...
func (a *app) loadTeam(ctx context.Context, id string, accountID int64, day string) (team, error) {
	var t team
	var leader int64
	if err := a.db.QueryRowContext(ctx, `SELECT id,boss_type,invite_code,server,leader_account_id
FROM player_teams WHERE id=? AND day_key=? AND status='active'`, id, day).Scan(&t.ID, &t.BossType, &t.InviteCode, &t.Server, &leader); err != nil {
		return t, err
	}
	t.Leader = leader == accountID
	rows, err := a.db.QueryContext(ctx, `SELECT m.character_id,m.account_id
FROM player_team_members m
WHERE m.team_id=? ORDER BY m.joined_at`, id)
	...
	for rows.Next() {
		...
		var memberID int64
		if rows.Scan(&characterID, &memberID) == nil {
			t.Members = append(t.Members, teamMember{CharacterID: characterID, IsLeader: memberID == leader})
		}
	}
	...
	pending, err := a.db.QueryContext(ctx, `SELECT p.id,p.character_id,p.requested_at
FROM player_team_applications p
WHERE p.team_id=? AND p.day_key=? AND p.status='pending' ORDER BY p.requested_at`, id, day)
	if err != nil {
		return t, err
	}
	defer pending.Close()
	for pending.Next() {
		...
		if pending.Scan(&request.ID, &request.CharacterID, &request.RequestedAt) == nil {
...
WHERE m.target_team_id=? AND m.day_key=? AND m.status='pending' ORDER BY m.requested_at`, id, day)
	if err != nil {
		return t, err
	}
	defer merges.Close()
...
func (a *app) applications(ctx context.Context, accountID int64, day string, history bool) ([]teamApplication, error) {
	query := `SELECT p.id,p.boss_type,t.invite_code,p.character_id,p.status,COALESCE(p.decision_reason,''),p.day_key,p.requested_at
FROM player_team_applications p JOIN player_teams t ON t.id=p.team_id WHERE p.account_id=?`
	...
	for rows.Next() {
		...
		if rows.Scan(&item.ID, &item.BossType, &item.InviteCode, &item.CharacterID, &item.Status, &item.Reason, &item.DayKey, &item.RequestedAt) == nil {
...
SELECT ?,?,?,?,?,? WHERE (SELECT COUNT(*) FROM player_team_members WHERE team_id=?) < 10`, teamID, accountID, boss, characterID, day, joinedAt, teamID)
	...
	if _, err = tx.ExecContext(r.Context(), "UPDATE player_team_applications SET status='approved',decided_at=?,decision_reason=NULL WHERE id=?", joinedAt, in.RequestID); err != nil {
		writeErr(w, errors.New("internal-error"))
		return
	}
	if err = closePendingApplications(r.Context(), tx, accountID, boss, day, in.RequestID, applicationReasonJoinedOtherTeam, joinedAt); err != nil {
		writeErr(w, errors.New("internal-error"))
		return
	}
	...
	t, err := a.loadTeam(r.Context(), teamID, leader.ID, day)
	if err != nil {
		writeErr(w, errors.New("internal-error"))
		return
	}
...
func (a *app) rejectJoin(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		writeErr(w, errors.New("method-not-allowed"))
		return
	}
	...
	defer tx.Rollback()
	var teamID string
	err = tx.QueryRowContext(r.Context(), `SELECT p.team_id FROM player_team_applications p
JOIN player_teams t ON t.id=p.team_id
WHERE p.id=? AND p.status='pending' AND p.day_key=? AND t.leader_account_id=? AND t.status='active'`, in.RequestID, day, leader.ID).Scan(&teamID)
	if errors.Is(err, sql.ErrNoRows) {
		writeErr(w, errors.New("request-not-found"))
		return
	}
...
SET status='rejected',decided_at=?,decision_reason=? WHERE id=?`, decidedAt, applicationReasonLeaderRejected, in.RequestID); err != nil {
		writeErr(w, errors.New("internal-error"))
		return
	}
	if err = tx.Commit(); err != nil {
