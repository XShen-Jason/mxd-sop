...
func (a *app) previewJoin(w http.ResponseWriter, r *http.Request) {
	...
	var status string
	if err = a.db.QueryRowContext(r.Context(), `SELECT status FROM player_team_applications WHERE team_id=? AND account_id=?`, teamID, ac.ID).Scan(&status); err == nil && status == "approved" {
