...
WHERE t.status='active' AND m.account_id=? AND m.day_key=? ORDER BY t.created_at DESC`, ac.ID, day)
	...
	for rows.Next() {
		var id string
		if rows.Scan(&id) == nil {
			if t, loadErr := a.loadTeam(r.Context(), id, ac.ID); loadErr == nil {
...
(team_id,account_id,boss_type,character_id,day_key,joined_at) VALUES(?,?,?,?,?,?)`, id, ac.ID, in.BossType, in.CharacterID, day, time.Now().UTC().Format(time.RFC3339)); err != nil {
	...
	t, err := a.loadTeam(r.Context(), id, ac.ID)
	if err != nil {
		writeErr(w, errors.New("internal-error"))
		return
	}
...
func (a *app) join(w http.ResponseWriter, r *http.Request) {
	...
	var existing string
	err = tx.QueryRowContext(r.Context(), `SELECT status FROM player_team_applications WHERE team_id=? AND account_id=?`, teamID, ac.ID).Scan(&existing)
	if err == nil {
...
WHERE leader_account_id=? AND boss_type=? AND day_key=?`, accountID, boss, day).Scan(&found)
	if errors.Is(err, sql.ErrNoRows) {
		return false, nil
	}
	return err == nil, err
